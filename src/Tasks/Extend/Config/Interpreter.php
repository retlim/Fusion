...
namespace Valvoid\Fusion\Tasks\Extend\Config;
...
/**
 * Extend task config interpreter.
 */
class Interpreter implements ConfigInterpreter
{
    ...
    public function interpret(array $breadcrumb, mixed $entry): void
    {
        ...
        elseif (is_array($entry))
            ...
            $this->box->get(ConfigEvent::class,
                message: "The value must be the default \"" . Extend::class .
    ...
    private function interpretDefaultTask(array $breadcrumb, mixed $entry): void
    {
        if ($entry !== Extend::class)
            $this->bus->broadcast(
                $this->box->get(ConfigEvent::class,
                    message: "The value must be the \"" . Extend::class .
                    "\" class name string.",
                    level: Level::ERROR,
                    breadcrumb: $breadcrumb
                ));
