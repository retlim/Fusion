...
namespace Valvoid\Fusion\Tasks\Inflate\Config;
...
/**
 * Inflate task config interpreter.
 */
class Interpreter implements ConfigInterpreter
{
    ...
    /**
     * Interprets the inflate task config.
     *
    ...
    public function interpret(array $breadcrumb, mixed $entry): void
    {
        ...
        elseif (is_array($entry))
            ...
            $this->box->get(ConfigEvent::class,
                message: "The value must be the default \"" . Inflate::class .
    ...
    {
        if ($entry !== Inflate::class)
            $this->bus->broadcast(
                $this->box->get(ConfigEvent::class,
                    message: "The value must be the \"" . Inflate::class .
                    "\" class name string.",
                    level: Level::ERROR,
                    breadcrumb: $breadcrumb
                ));
        ...
        if ($entry !== Inflate::class)
            $this->bus->broadcast(
                $this->box->get(ConfigEvent::class,
                    ...
                    "index must be the \"" . Inflate::class . "\" string.",
                    level: Level::ERROR,
