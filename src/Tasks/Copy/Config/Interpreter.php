...
namespace Valvoid\Fusion\Tasks\Copy\Config;
...
class Interpreter implements ConfigInterpreter
{
    ...
    public function interpret(array $breadcrumb, mixed $entry): void
    {
        ...
        elseif (is_array($entry))
            ...
            $this->box->get(ConfigEvent::class,
                message: "The value must be the default \"" . Copy::class .
    ...
    private function interpretDefaultTask(array $breadcrumb, mixed $entry): void
    {
        if ($entry !== Copy::class)
            $this->bus->broadcast(
                $this->box->get(ConfigEvent::class,
                    message: "The value must be the \"" . Copy::class .
                    "\" class name string.",
                    level: Level::ERROR,
                    breadcrumb: $breadcrumb
                ));
        ...
        if ($entry !== Copy::class)
            $this->bus->broadcast(
                $this->box->get(ConfigEvent::class,
                    ...
                    "index must be the \"" . Copy::class . "\" string.",
                    level: Level::ERROR,
