...
namespace Valvoid\Fusion\Tasks\Shift\Config;
...
class Interpreter implements ConfigInterpreter
{
    ...
    public function interpret(array $breadcrumb, mixed $entry): void
    {
        ...
        elseif (is_array($entry))
            ...
            $this->box->get(ConfigEvent::class,
                message: "The value must be the default \"" . Shift::class .
    ...
    {
        if ($entry !== Shift::class)
            $this->bus->broadcast(
                $this->box->get(ConfigEvent::class,
                    message: "The value must be the \"" . Shift::class .
                    "\" class name string.",
                    level: Level::ERROR,
                    breadcrumb: $breadcrumb
                ));
        ...
        if ($entry !== Shift::class)
            $this->bus->broadcast(
                $this->box->get(ConfigEvent::class,
                    ...
                    "index must be the \"" . Shift::class . "\" string.",
                    level: Level::ERROR,
