...
namespace Valvoid\Fusion\Tasks\Snap\Config;
...
class Interpreter implements ConfigInterpreter
{
    ...
    public function interpret(array $breadcrumb, mixed $entry): void
    {
        ...
        elseif (is_array($entry))
            ...
            $this->box->get(ConfigEvent::class,
                message: "The value must be the default \"" . Snap::class .
    ...
    private function interpretDefaultTask(array $breadcrumb, mixed $entry): void
    {
        if ($entry !== Snap::class)
            $this->bus->broadcast(
                $this->box->get(ConfigEvent::class,
                    message: "The value must be the \"" . Snap::class .
                    "\" class name string.",
                    level: Level::ERROR,
                    breadcrumb: $breadcrumb
                ));
        ...
        if ($entry !== Snap::class)
            $this->bus->broadcast(
                $this->box->get(ConfigEvent::class,
                    ...
                    "index must be the \"" . Snap::class . "\" string.",
                    level: Level::ERROR,
