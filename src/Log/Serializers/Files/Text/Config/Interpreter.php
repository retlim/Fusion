...
class Interpreter implements ConfigInterpreter
{
    ...
    /**
     * Interprets the text serializer config.
     *
     * @param array $breadcrumb Index path inside the config to the text config.
    ...
    public function interpret(array $breadcrumb, mixed $entry): void
    {
        ...
        elseif (is_array($entry))
            ...
            $this->box->get(ConfigEvent::class,
                message: "The value must be the default '" . Text::class .
    ...
    private function interpretDefaultSerializer(array $breadcrumb, mixed $entry): void
    {
        if ($entry !== Text::class)
            $this->bus->broadcast(
                $this->box->get(ConfigEvent::class,
                    message: "The value must be the '" . Text::class .
                    "' class name string.",
                    level: Level::ERROR,
                    breadcrumb: $breadcrumb
                ));
    ...
    {
        // overlay reset value
        if ($entry === null || $entry === Text::class)
            return;

        $this->bus->broadcast(
            $this->box->get(ConfigEvent::class,
                message: "The value, serializer class name, of the 'serializer' " .
                "index must be the '" . Text::class . "' string.",
                level: Level::ERROR,
