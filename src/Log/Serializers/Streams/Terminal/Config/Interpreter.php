...
/**
 * Terminal stream log config interpreter.
 */
class Interpreter implements ConfigInterpreter
{
    ...
    /**
     * Interprets the terminal serializer config.
     *
     * @param array $breadcrumb Index path inside the config to the terminal config.
    ...
    public function interpret(array $breadcrumb, mixed $entry): void
    {
        ...
        elseif (is_array($entry))
            ...
            $this->box->get(ConfigEvent::class,
                message: "The value must be the default '" . Terminal::class .
    ...
    {
        if ($entry !== Terminal::class)
            $this->bus->broadcast(
                $this->box->get(ConfigEvent::class,
                    message: "The value must be the '" . Terminal::class .
                    "' class name string.",
                    level: Level::ERROR,
                    breadcrumb: $breadcrumb
                ));
    ...
    {
        // overlay reset value
        if ($entry === null || $entry === Terminal::class)
            return;

        $this->bus->broadcast(
            $this->box->get(ConfigEvent::class,
                message: "The value, serializer class name, of the 'serializer' " .
                "index must be the '" . Terminal::class . "' string.",
                level: Level::ERROR,
